...
void StoredGridForecastQueryHandler::query(const StoredQuery& stored_query, const std::string& language, std::ostream& output) const
{
  try
  {
    ...
    StoredForecastQueryHandler::Query query;
    ...
    try
    {
      ...
      try
      {
        ...
        auto transformation = plugin_data.get_crs_registry().create_transformation("urn:ogc:def:crs:EPSG::4326", crs);
        ...
        hash["responseTimestamp"] = Fmi::to_iso_extended_string(get_plugin_data().get_time_stamp()) + "Z";
...
Table_sptr StoredGridForecastQueryHandler::extract_forecast(Query& wfsQuery) const
{
  try
  {
    ...
    for (auto tloc = wfsQuery.locations.begin(); tloc != wfsQuery.locations.end(); ++tloc)
    {
      ...
      attributeList.addAttribute("type",std::to_string(QueryServer::Query::Type::PointValues));

      attributeList.addAttribute("locationType",std::to_string(QueryServer::Query::LocationType::Point));
    ...
    boost::optional<std::string> template_file_name)
{
  try
  {
    StoredGridForecastQueryHandler* qh = new StoredGridForecastQueryHandler(reactor, config, plugin_data, template_file_name);
